...
class TestManageCloudflare(unittest.TestCase):
    ...
    def test_load_config_specific(self, mock_env_get):
    ...
    @patch('os.environ.get')
    def test_load_config_generic_fallback(self, mock_env_get):
        def side_effect(key, default=None):
            env = {
                "CLOUDFLARE_ZONE_ID": "generic_zone_123",
        ...
        self.assertEqual(config["zones"]["fallback-domain.com"], "generic_zone_123")
        self.assertEqual(len(config["zones"]), 1)
        self.assertEqual(config["account_id"], "acc_id")

    @patch('os.environ.get')
    def test_load_config_generic_fallback_default_domain(self, mock_env_get):
        def side_effect(key, default=None):
            env = {
                "CLOUDFLARE_ZONE_ID": "generic_zone_456",
                "CLOUDFLARE_ACCOUNT_ID": "acc_id",
                "CLOUDFLARE_API_TOKEN": "api_tok"
            }
            # Simulate DOMAIN_NAME being missing
            return env.get(key, default)
        ...
        self.assertEqual(config["zones"]["unknown-domain"], "generic_zone_456")
        self.assertEqual(len(config["zones"]), 1)

    @patch('os.environ.get')
    def test_load_config_lengkundee_via_generic_id(self, mock_env_get):
        def side_effect(key, default=None):
            env = {
                "CLOUDFLARE_ZONE_ID": "gen_id_for_leng",
        ...
        self.assertEqual(config["zones"]["lengkundee01.org"], "gen_id_for_leng")
        self.assertEqual(len(config["zones"]), 1)
