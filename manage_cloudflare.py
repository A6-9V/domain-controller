...
def load_config():
    ...
    val = env.get("CLOUDFLARE_ZONE_ID_LENGKUNDEE")
    if val:
        zones["lengkundee01.org"] = val
    else:
        val = env.get("CLOUDFLARE_ZONE_ID")
        if val and env.get("DOMAIN_NAME") == "lengkundee01.org":
            zones["lengkundee01.org"] = val

    val = env.get("CLOUDFLARE_ZONE_ID_GENXFX")
    if val:
        zones["genxfx.org"] = val

    # Generic fallback
    if not zones:
        val = env.get("CLOUDFLARE_ZONE_ID")
        if val:
            domain = env.get("DOMAIN_NAME", "unknown-domain")
            zones[domain] = val
